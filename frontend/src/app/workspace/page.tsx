...
const SAMPLE_IMAGES = [
  { name: "Cell Structure", path: "/samples/sample_cell.png" },
  { name: "Circuit Diagram", path: "/samples/sample_circuit.png" },
  { name: "Ray Diagram", path: "/samples/sample_optics.png" },
];

export default function WorkspacePage() {
  const { result, progress, error, processImage, stage } =
    useDiagramPipeline();
  ...
  const handleSample = async (path: string) => {
    try {
      ...
      const file = new File([blob], path.split("/").pop() || "sample.png", {
        type: blob.type,
      });
      handleFile(file);
    } catch (err) {
  ...
  const confidence = result?.semantic_regions?.length
    ? Math.round(
        (result.semantic_regions.reduce((sum, r) => sum + r.confidence, 0) /
  ...
  const statusText = () => {
    ...
    if (stage === "segmenting") return "DETECTING REGIONS";
    if (stage === "simplifying") return "SIMPLIFYING PATHS";
    if (stage === "tactile") return "GENERATING TACTILE MAP";
    ...
    <main className="min-h-screen bg-[#070706] text-[#e8e2d6] overflow-hidden">
      <nav className="h-20 border-b border-white/10 flex items-center justify-between px-10">
        <div className="flex items-center gap-3">
          ...
          <span className="font-serif text-xl tracking-wide">TactileGen</span>
        </div>
        ...
        <div className="hidden md:flex gap-12 text-xs tracking-[0.25em] text-zinc-500">
          ...
          <span>PATHS {result?.tactile_paths?.length ?? "--"}</span>
          <span>LABELS {result?.extracted_labels?.length ?? "--"}</span>
      ...
      <section className="grid lg:grid-cols-[420px_1fr] gap-16 px-16 py-20">
        <div className="flex flex-col justify-center">
          ...
          <h1 className="font-serif text-7xl leading-[0.95]">
            Make every
            <br />
            <span className="italic text-orange-400">diagram</span>
            <br />
            touchable.
          </h1>
            ...
            <div className="flex flex-col gap-3">
              {SAMPLE_IMAGES.map((sample) => (
                <button
                  ...
                  onClick={() => handleSample(sample.path)}
                >
                  {sample.name}
                </button>
              ))}
          ...
          <div className="mt-24 border-l border-orange-400/30 pl-6">
            <p className="text-xs text-orange-400">01</p>
            <h3 className="mt-3 font-serif text-xl">ORIGINAL IMAGE</h3>
        ...
        <div className="border border-white/10 bg-[#0b0b09] p-8">
          <div className="flex justify-between text-xs tracking-widest text-zinc-500 mb-8">
            ...
            <span className="text-orange-400">{statusText()}</span>
          </div>
          ...
          <div className="aspect-video border border-white/10 bg-[#151512] flex items-center justify-center relative overflow-hidden">
            {viewMode === "original" && imageUrl ? (
              <img src={imageUrl} className="max-h-full object-contain" />
              ...
              <div className="text-zinc-600">Upload diagram to begin</div>
            )}
          ...
          <div className="mt-8">
            ...
            <div className="flex justify-between mt-6 text-xs tracking-widest text-zinc-500">
              <span
                className={stage === "uploading" ? "text-orange-400" : ""}
              >
                01 ORIGINAL
              </span>
              <span
                className={
                  stage === "understanding" || stage === "segmenting"
                    ? "text-orange-400"
                    : ""
                }
              >
              ...
              <span
                className={stage === "simplifying" ? "text-orange-400" : ""}
              >
                03 SIMPLIFY
              </span>
              <span
                className={
                  stage === "tactile" || stage === "complete"
                    ? "text-orange-400"
                    : ""
                }
              >
      ...
      <footer className="border-t border-white/10 px-16 py-6 flex justify-between text-xs tracking-widest text-zinc-500">
        ...
        <Link href="/workspace" className="text-orange-400">
          EXPLORE WORKSPACE →
        </Link>
      </footer>
    </main>
